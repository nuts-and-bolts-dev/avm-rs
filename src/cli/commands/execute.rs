...
use crate::cli::{ExecuteCommand, GlobalOptions, InputType, ExecutionMode};
...
fn auto_detect_and_load(input: &str) -> Result<Vec<u8>> {
    ...
    if input.chars().all(|c| c.is_ascii_hexdigit() || c.is_whitespace()) && input.len() > 10 {
...
fn execute_normal(
    ...
    let result = vm.execute(bytecode, config.clone(), ledger)
